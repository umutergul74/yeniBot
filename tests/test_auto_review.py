...
def _write_minimal_report(path, *, missing_selected: bool = False, future_oos_ready: bool = False) -> None:
    ...
    ).to_csv(path / "sample_weight_audit.csv", index=False)
    pd.DataFrame(
        [
            {
                "profile": control,
                ...
                "component": "combined",
                "fold_count": 1,
                "mean_effective_sample_fraction": 1.0,
            }
        ]
    ).to_csv(path / "sample_weight_audit_summary.csv", index=False)
    pd.DataFrame(
        [
            {
                "profile": control,
    ...
    ).to_csv(path / "multitask_gradient_audit.csv", index=False)
    pd.DataFrame(
        [
            {
                "profile": control,
    ...
    ).to_csv(path / "multitask_gradient_audit_summary.csv", index=False)
    pd.DataFrame(
        [
            {
