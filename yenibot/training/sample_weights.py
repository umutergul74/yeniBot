    ...
    *,
    ...
    if valid.size == 0:
        ...
        stats = {
            ...
            "p10_weight": float(np.quantile(valid, 0.10)),
            "p50_weight": float(np.quantile(valid, 0.50)),
            "p90_weight": float(np.quantile(valid, 0.90)),
...
def event_strength_weights(
    ...
    *,
    ...
    n = int(len(frame))
    if n == 0:
        return np.asarray([], dtype="float32"), [], "empty_frame"
    ...
    if not selected:
        return np.ones(n, dtype="float32"), [], "no_matching_event_columns"
    ...
    for column in selected:
        ...
        scores.append(ranks.astype(float))
    if not scores:
        return np.ones(n, dtype="float32"), selected, "event_columns_not_rankable"

    event_score = pd.concat(scores, axis=1).mean(axis=1).to_numpy(dtype=float)
    ...
    return weights.astype("float32"), selected, "ok"
...
def build_sample_weights(
    ...
    if event_enabled:
        event_weights, event_columns, event_notes = event_strength_weights(
            train_frame,
            feature_columns,
